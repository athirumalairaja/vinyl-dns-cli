...
METHOD = 'GET'
BODY = {}
def search_record_set_info(args):
    try:
        params = {'recordNameFilter': ''.join([args.record_set_name, '.', args.domain, '*',]), 'type':'A'}
        path = ''.join(['/recordsets?', 'recordNameFilter=', args.record_set_name, '.', args.domain, '*', '&type=A'])
